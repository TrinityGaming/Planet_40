...
namespace Planet
{
  public class TerrainGeometrySurface
  {
    ...
    {
      if (HeightData == null)
        HeightData = new float[totalWidth * totalHeight];
    ...
    public void Save(TerrainNodeSplitItem item)
    {
      ...
      Color[] data = new Color[totalWidth * totalHeight];
...
//#if ! XBOX
//      Texture2D tex = new Texture2D(device, totalWidth, totalHeight, false, SurfaceFormat.Color);
//      tex.SetData<Color>(data);

//      using (FileStream stream = new FileStream(String.Format(@"screenshots\geometry_{0}_{1}.png", Globals.NormalMapNumber++, item.ChildIndex), FileMode.Create))
//      {
